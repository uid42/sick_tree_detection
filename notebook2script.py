...
def notebook2script(fname=None, outputDir=None, allFiles=None, upTo=None, ):
    ...
    if not allFiles: 
        notebook2scriptSingle(fname, outputDir)
    else:
        print('Begin...')
        [notebook2scriptSingle(f, outputDir) for f in getSortedFiles(allFiles,upTo)]
        print('...End')
        
        
def notebook2scriptSingle(fname, outputDir):
    "Finds cells starting with `#export` and puts them into a new module"
    fname = Path(fname)
...
'''
    ...
    if outputDir is None:
        ourputDir = fname.parent/'exp'
    if not outputDir.exists(): outputDir.mkdir()
    output_path = outputDir/fname_out
